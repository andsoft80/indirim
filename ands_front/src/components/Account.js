...
export default function Account() {
    ...
    const getCompanyUsers = (id) => {
        ...
        axios.get(be_conf.server + '/companyusers', { headers: { "Authorization": 'Bearer ' + Authcontrol.getToken() }, params: { companyid: id } })
    ...
    const handleAddCompany = () => {
        ...
        if (name == '' || inn == '') {
            ...
            axios.post(be_conf.server + '/company', {
                name: name,
                inn: inn,
                kpp: kpp,
                address: address,
    ...
    const formSub = (e) => {
        ...
        axios.post(be_conf.server + '/table/companies/action/put', {
            id: companyData.id,
            name: name,
            inn: inn,
            kpp: kpp,
    ...
    const [haveSelected, setHaveSelected] = useState(false);//have selected company users
    const [companyTrx, setCompanyTrx] = useState([]);
    const [companyBalance, setCompanyBalance] = useState(0);


    const getCompanyTrx = (id) => {
        axios.post(be_conf.server + '/table/free/action/sql', {
    ...
    const deleteCompanyUsersById = (id) => {
        axios.post(be_conf.server + '/table/users/action/delete', {
            id: id
    ...
    const deleteCompanyUsers = () => {
        ...
        if (selected[userData.id]) {
            ...
            if (window.confirm("Удалить все выделенные записи?")) {
